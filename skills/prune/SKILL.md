...
**`prune` acts as an evidentiary garbage collector and architectural reconciler.**
...
**`prune` flips this assumption entirely:**
...
If the user passed a specific target in their initial prompt (e.g. `prune HEAD~2..HEAD` or `prune src/auth/`), confirm that target directly.
