...
description: >-
  ...
  product, a change to the current product, an adjacent deviation, or still
  deliberately open; uncover what the user is really trying to achieve; research
...
```mermaid
flowchart LR
    A[Understand intent and place the idea] --> B[Explore what matters now]
    B --> C[Reflect evidence and reframe]
    C --> D[Steer to the next useful move]
    D --> A
    D -. when useful .-> E[Decision or artifact]
```
