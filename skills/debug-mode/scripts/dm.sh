# debug-mode skill -- `dm` command (bash + zsh).
#
# Source this file from your shell rc, e.g.:
#   source /path/to/debug-mode/scripts/dm.sh
# or run the installer once:
#   sh /path/to/debug-mode/scripts/install-dm.sh
#
# Usage:
...
dm() {
	...
	if [ ! -f "$script" ]; then
		printf 'dm: debug-mode launcher not found at %s\n' "$script" >&2
		return 1
	fi
	if [ "$#" -eq 0 ]; then
